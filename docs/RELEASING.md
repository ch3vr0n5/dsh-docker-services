...
3. Run `npm run test:container`. Inspect all four artifacts; each must contain
   ...
   import plugin client/controller/proxy exports and resolve both bins. Set
   `DSH_DASHBOARD_TARBALL` to the exact admitted Dashboard artifact and rerun
   the consumer test before widening or retaining the Dashboard peer range.
4. Sign/tag the reviewed commit and publish shared, controller, proxy, then plugin.
